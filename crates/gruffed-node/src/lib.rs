...
fn registry() -> &'static Mutex<GraphRegistry> {
  GRAPH_REGISTRY.get_or_init(|| Mutex::new(GraphRegistry { next_id: 1, graphs: HashMap::new() }))
}
...
pub fn build_module_graph(root: String, config_json: Option<String>) -> Result<BuildResultJs> {
  let config = if let Some(json) = config_json {
    GruffedConfig::from_jsonc(&json).map_err(|e| Error::new(Status::InvalidArg, e.to_string()))?
  } else {
  ...
  let handle = {
    let mut reg = registry().lock().map_err(|e| Error::new(Status::GenericFailure, e.to_string()))?;
    let id = reg.next_id;
    reg.next_id += 1;
...
pub fn analyze_graph(
  ...
  let report = {
    let reg = registry().lock().map_err(|e| Error::new(Status::GenericFailure, e.to_string()))?;
    let graph = reg
      .graphs
      .get(&graph_handle)
      .ok_or_else(|| Error::new(Status::InvalidArg, format!("unknown graph handle {}", graph_handle)))?;
