...
const projectsRoot =
  process.env.GRUFFED_TRACE_PROJECTS_ROOT ?? path.join(process.env.HOME ?? "", "Projects");
...
for (const repository of repositories) {
  ...
  for (const variant of [
    ...
    if (result.error || !result.stdout.trim()) {
      throw (
        result.error ??
        new Error(`Trace generation produced no output for ${repository.name}: ${result.stderr}`)
      );
    }
    ...
    console.log(
      `${repository.name}${variant.suffix}: ${trace.stats.totalNodes} modules, ${trace.stats.totalEdges} imports, ${trace.stats.projectedNodes} visible nodes`,
    );
  }
}
