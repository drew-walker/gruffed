      ...
      <div className={styles.inner}>
        ...
        <section className={styles.scanSummary} aria-labelledby="scan-summary-heading">
          <div className={styles.summaryHeader}>
            ...
            <p>
              The detailed explorer above uses newly pinned trace artifacts. This table preserves
              the wider release-readiness scan for quick comparison without opening every graph.
            </p>
          </div>
          <div className={styles.summaryStats} aria-label="Public scan totals">
            <span>
              <strong>{demoRepoScanSummary.repoCount}</strong> repositories
            </span>
            <span>
              <strong>{formatNumber.format(demoRepoScanSummary.totalNodes)}</strong> modules
            </span>
            <span>
              <strong>{formatNumber.format(demoRepoScanSummary.totalEdges)}</strong> imports
            </span>
            <span>
              <strong>{(demoRepoScanSummary.totalElapsedMs / 1000).toFixed(1)}s</strong> combined
              runtime
            </span>
          </div>
                  ...
                  <tr key={result.repo}>
                    <th scope="row">
                      <div className={styles.repoCell}>
                        <img
                          src={`https://github.com/${result.repo.split("/")[0]}.png?size=64`}
                          width="30"
                          height="30"
                          loading="lazy"
                          alt=""
          ...
          <p className={styles.summaryNote}>
            Broader snapshot generated {demoRepoScanSummary.generatedAt}; retained as a coarse
            parser-coverage summary rather than a controlled benchmark.
          </p>
        </section>
      </div>
    </main>
