...
const traceLoaders: Record<RepoName, () => Promise<TraceData>> = {
  Babel: () =>
    import("../content/traces/babel-full.json").then((module) => module.default as TraceData),
  TypeScript: () =>
    import("../content/traces/typescript-full.json").then((module) => module.default as TraceData),
  Svelte: () =>
    import("../content/traces/svelte-full.json").then((module) => module.default as TraceData),
};
  ...
  return (
    <section className={styles.section} aria-labelledby="public-repo-heading">
      <div className={styles.header}>
        ...
        <p>
          Ranked structural paths extracted from complete scans of large open-source TypeScript and
          JavaScript repositories, generated with the installable CLI and the same Rust graph
          builder.
        </p>
      </div>
      ...
      <div className={styles.tabs} role="tablist" aria-label="Featured public repositories">
        {featured.map((item) => (
          <button
            key={item.name}
            role="tab"
            aria-selected={item.name === repo.name}
            onClick={() => selectRepo(item.name)}
          >
            {item.name}
          </button>
        ))}
      ...
      <div className={styles.panel}>
        <div className={styles.repoIdentity}>
          ...
          <a
            href={`https://github.com/${repo.repo}/tree/${repo.commit}`}
            target="_blank"
            rel="noreferrer"
          >
            View pinned source <ExternalLink size={14} />
          </a>
        </div>
        ...
        <div className={styles.metrics}>
          <article>
            ...
            <strong>{trace ? format.format(trace.stats.totalNodes) : "—"}</strong>
          </article>
          <article>
            <GitFork size={18} />
            <span>Import edges</span>
            <strong>{trace ? format.format(trace.stats.totalEdges) : "—"}</strong>
          </article>
          <article>
            <Timer size={18} />
            <span>Graph build</span>
            <strong>{trace ? `${trace.stats.buildTimeMs.toLocaleString()}ms` : "—"}</strong>
          </article>
        </div>

        <div className={styles.pathExplorer} aria-busy={!trace}>
          {!trace ? (
            <div className={styles.traceLoading}>Loading pinned structural paths…</div>
          ) : (
            <>
              <div className={styles.pathHeader}>
                <div>
                  <span>Structural paths</span>
                  <strong>Show the problems, not the entire graph.</strong>
                </div>
                <div className={styles.pathToggle} role="tablist" aria-label="Structural path type">
                  <button
                    type="button"
                    role="tab"
                    aria-selected={pathKind === "cycles"}
                    onClick={() => {
                      setPathKind("cycles");
                      setSelectedPath(0);
                    }}
                  >
                    <RotateCcw size={14} /> Cycles <b>{cycles.length}</b>
                  </button>
                  <button
                    type="button"
                    role="tab"
                    aria-selected={pathKind === "chains"}
                    onClick={() => {
                      setPathKind("chains");
                      setSelectedPath(0);
                    }}
                  >
                    <Route size={14} /> Longest chains <b>{chains.length}</b>
                  </button>
                </div>
              </div>
              <div className={styles.pathBody}>
                <ol
                  ...
                  aria-label={
                    pathKind === "cycles"
                      ? "Largest circular dependencies"
                      : "Longest dependency chains"
                  }
                >
                  {paths.map((path, index) => (
                    <li key={`${path.kind}-${path.nodes.join("-")}`}>
                      <button
                        type="button"
                        aria-pressed={index === selectedPath}
                        onClick={() => setSelectedPath(index)}
                      >
                        <span>{String(index + 1).padStart(2, "0")}</span>
                        <strong>
                          {path.kind === "cycle"
                            ? `${path.nodes.length}-module cycle`
                            : `${path.nodes.length - 1}-hop chain`}
                        </strong>
                        <small>
                          {shortPath(path.nodes[0])} → {shortPath(path.nodes.at(-1) ?? "")}
                        </small>
                      </button>
                    </li>
                  ))}
                ...
                <div className={styles.pathDetail}>
                  {activePath ? (
                    <>
                      <div className={styles.pathDetailHeader}>
                        <span>
                          {activePath.kind === "cycle" ? "Circular dependency" : "Dependency depth"}
                        </span>
                        <strong>{activePath.nodes.length} modules</strong>
                        <small>
                          {activePath.kind === "cycle"
                            ? "Following the final import returns to the first module."
                            : "A deepest acyclic route through the repository graph."}
                        </small>
                      </div>
                      ...
                      {activePath.kind === "cycle" && (
                        <div className={styles.returnEdge}>
                          <RotateCcw size={14} /> returns to {shortPath(activePath.nodes[0])}
                        </div>
                      )}
                    </>
                  ) : (
                    <p className={styles.emptyPath}>
                      No {pathKind === "cycles" ? "cycles" : "dependency chains"} were found in this
                      scan.
                    </p>
                  )}
                </div>
        ...
        <p className={styles.note}>
          Pinned at commit {repo.commit}. Cycles are confirmed analyzer findings. Longest chains are
          derived deterministically from the complete graph after collapsing strongly connected
          components.
        </p>
      </div>
    </section>
...
function cyclePaths(trace: TraceData): StructuralPath[] {
  return trace.findings
    .filter((finding) => finding.ruleId === "no-cycles")
    .sort((a, b) => b.path.length - a.path.length || a.file.localeCompare(b.file))
    .map((finding) => ({ kind: "cycle", nodes: finding.path }));
}
...
function longestChains(trace: TraceData): StructuralPath[] {
  ...
  components.forEach((members, component) =>
    members.forEach((id) => componentOf.set(id, component)),
  );
  ...
  const bestFrom = (component: number): number[] => {
    ...
    let best = [component];
    for (const next of dag.get(component) ?? []) {
      const candidate = [component, ...bestFrom(next)];
      if (candidate.length > best.length) best = candidate;
    }
    memo.set(component, best);
    return best;
  };
  const candidates = components
    .map((_, component) => bestFrom(component))
...
function materializePath(
  ...
  for (let index = 0; index < componentPath.length - 1; index++) {
    const sourceComponent = componentPath[index];
    const targetComponent = componentPath[index + 1];
    const bridge = edges.find(
      (edge) =>
        componentOf.get(edge.source) === sourceComponent &&
        componentOf.get(edge.target) === targetComponent,
    );
    if (!bridge) continue;
    if (result.length === 0) result.push(bridge.source);
    const current = result.at(-1);
    if (current && current !== bridge.source)
      result.push(
        ...pathWithinComponent(
          current,
          bridge.source,
...
function stronglyConnectedComponents(
  ...
  const visit = (id: string) => {
    ...
    for (const next of outgoing.get(id) ?? []) {
      ...
      } else if (onStack.has(next)) low.set(id, Math.min(low.get(id) ?? 0, index.get(next) ?? 0));
    }
